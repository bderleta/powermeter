...
def get_metrics_finder(client, meterAddr):
    m = ""
    val = client.read_input_registers(address=105, count=4, slave=meterAddr)
    if (not val.isError()):
    ...
    val = client.read_input_registers(address=140, count=2, slave=meterAddr)
    if (not val.isError()):
        value = from_T6(val.registers)
        m += "powermeter_%s{address=\"%03u\"} %g\n" % ("power_active", meterAddr, value)
    val = client.read_input_registers(address=462, count=8, slave=meterAddr)
    if (not val.isError()):
...
def get_metrics_eastron(client, meterAddr):
    m = ""
    val = client.read_input_registers(address=0, count=2, slave=meterAddr)
    if (not val.isError()):
        value = from_modbus_float(val.registers)
        m += "powermeter_%s{address=\"%03u\"} %g\n" % ("voltage", meterAddr, value)
    val = client.read_input_registers(address=12, count=2, slave=meterAddr)
    if (not val.isError()):
        value = from_modbus_float(val.registers)
        m += "powermeter_%s{address=\"%03u\"} %g\n" % ("power_active", meterAddr, value)
    val = client.read_input_registers(address=70, count=10, slave=meterAddr)
    if (not val.isError()):
        ...
        try:
            ...
            if meterType == "finder":
                ...
                for register in registers[meterType]:
                    val = client.read_input_registers(address=register[0], count=register[1], slave=meterAddr)
                    if (not val.isError()):
